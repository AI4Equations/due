...
# SDE Parameters: dX_t = mu * X_t dt + sigma * X_t dW_t
MU = 2.0          # Drift coefficient
SIGMA = 1.0       # Volatility (noise scale)

# Simulation Horizon & Volume
N_TRAJECTORIES = 50000  # Total number of independent training trajectories (H)
N_TEST_TRAJECTORIES = 50000 # Total number of independent test trajectories
T_TRAIN_HORIZON = 0.5          # Total time to simulate
T_TEST_HORIZON = 1 
DT = 0.01                # Time step size

# Initial State Distribution (Uniform Sampling)
X0_MIN = 0.0             # Lower bound for initial state
X0_MAX = 2.0             # Upper bound for initial state

# Output Configuration
OUTPUT_TRAIN_FILENAME = "1d_GBM_process_train.mat"
OUTPUT_TEST_FILENAME = "1d_GBM_process_test.mat"
# =======================================================================

def generate_gbm_trajectories(num_trajectories, dataset_type="Train"):
    print(f"--- Generating 1D Geometric Brownian Motion ({dataset_type}) ---")
    ...
    for t in range(1, num_steps):
        ...
        # Calculate the drift (deterministic pull)
        drift = MU * x_prev * DT
        
        # Calculate the diffusion (stochastic noise)
        diffusion = SIGMA * x_prev * sqrt_dt * noise
...
def save_to_mat(trajectories, filename):
    ...
    mat_dictionary = {
        ...
        "metadata": {
            "mu": MU,
            "sigma": SIGMA,
            "description": "1D Geometric Brownian Motion"
        }
    }
...
if __name__ == "__main__":
    ...
    train_data = generate_gbm_trajectories(N_TRAJECTORIES, "Train")
    ...
    test_data = generate_gbm_trajectories(N_TEST_TRAJECTORIES, "Test")
